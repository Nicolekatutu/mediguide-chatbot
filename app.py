...
df_doctors = pd.read_excel(r'doctor_consult.xlsx')    # Doctors dataset


def get_doctor_recommendation(disease):
    # Find the doctor recommendations for the given disease
    if disease in df_doctors['disease'].values:
        doctor_info = df_doctors[df_doctors['disease'] == disease]
        recommendations = []
        for index, row in doctor_info.iterrows():
            recommendations.append(f"{row['doctor']}")
        return f"Recommended doctors: {', '.join(recommendations)}"
    else:
        return "No doctor recommendations available for this disease."
...
def chat_msg():
    ...
    if currentState == 0:
        ...
        if user_message.lower() == 'yes':
            ...
            response += f"{doctor_recommendation}\n"
        ...
        if user_message.lower() == 'yes':
         ...
         if user_message.lower() == 'yes':
             ...
             response = f"Thanks for using Mediguide, {username}! Have a great day!"
       ...
       userSession[sessionId]=9
        ...
        if user_message.lower() == "yes":
            ...
            response = f"Thanks for using Mediguide,{username}!"
            userSession.pop(sessionId, None)
...
def generate_html_response(message, div_class="message sender"):
    ...
    return f'<div class="{div_class}" style="margin-bottom: 10px;"><br><div class="message-bubble">{message}</div></div>'
